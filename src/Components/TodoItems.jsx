  ...
  return (
    <div className='flex items-center my-3 gap-2'>
      ...
      <div onClick={()=>{toggle(id)}} className='flex flex-1 items-center cursor-pointer'>
        <img src={isComplete? tick_icon : not_tick_icon} alt="TickIcon" className='w-7'/>
        <p className={`text-slate-700 ml-4 text-[17px] decoration-slate-500 ${isComplete ? "line-through" : ""}`}>
            {text}
            </p>
      </div>
      <img onClick={()=>{deleteTodo(id)}} src={delete_icon} alt="DeleteIcon" className='w-3.5 cursor-pointer'/>
